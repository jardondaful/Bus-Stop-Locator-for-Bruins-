...
#add data to map
map_name = "Test"
aprx = arcpy.mp.ArcGISProject(os.path.join(FolderPath, "Bus_Locator_Bruins.aprx"))
m = aprx.listMaps("Test")[0]
m.addDataFromPath(os.path.join(FolderPath, "Streets.shp"))
aprx.saveACopy(os.path.join(FolderPath, "Bus_Locator_Bruins.aprx"))


# Check if the map already exists, and create a new one if it doesn't
#if map_name not in [m.name for m in aprx.listMaps()]:
   # aprx.listMaps().addMap(map_name)

# Get a reference to the map
#m = aprx.listMaps(map_name)[0]

#m.addDataFromPath(ProjectGDB)
